...
int macro() {
    ...
    TH1I *HTypes = new TH1I("Species counts data",
                            "#bf{Types of particles distribution}", 8, 0, 8);
    TH2D *HAngles = new TH2D("HAngles", "Angles distribution", 1000, 0, 2 * 3.5,
                             1000, 0, 3.5);
    TH1D *HP = new TH1D("Momentum module data", "#bf{Momentum distribution}",
                        200, 0, 10);
    TH1D *HTransverseImpulse =
        new TH1D("Transverse momentum module data",
                 "Transverse Momentum distribution", 200, 0, 10);
    ...
    TH1D *HMassPioneKaoneDiscorde = new TH1D(
        ...
        "Invariant Mass for opposite charged Pion and Kaon distribution", 40,
        0.7, 1.1);
    ...
    TH1D *HMassKDecay = new TH1D(
        "Decay IM data",
        "#bf{Invariant Mass for the children of each decay distribution}", 500,
        0.7, 1.1);
    HMassKDecay->Sumw2();
    int numNegativeCharge = 0;
    int numPositiveCharge = 0;
    ...
    TCanvas *canvas2 =
        new TCanvas("InvariantMassCanvas", "canvas2", 1000, 1000);
    canvas2->Divide(3, 2);
    canvas2->cd(1);
    HInvariantMass->Draw();
    ...
    HMassKDecay->GetXaxis()->CenterTitle(true);
    HMassKDecay->GetXaxis()->SetTitleSize(.05);
    HMassKDecay->GetXaxis()->SetTitle("Invariant Mass(GeV/c^2)");

    HMassKDecay->GetYaxis()->CenterTitle(true);
    HMassKDecay->GetYaxis()->SetTitleSize(.05);
    HMassKDecay->GetYaxis()->SetTitle("Counts");
